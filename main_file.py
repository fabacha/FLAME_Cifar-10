...
def get_net(net_type, num_inputs, num_outputs=10):
    ...
    if net_type == "lr":
        ...
        import models_flame.cnn_flame as cnn  # Import your CNN model
        net = cnn.CNN(num_classes=num_outputs)
        print(net)
...
def get_byz(byz_type):
    ...
    if byz_type == "no":
        ...
        return attacks_flame.min_max_attack
    ...
    with torch.no_grad():
        for i, (inputs, targets) in enumerate(data_iterator):
            ...
            if trigger:  # backdoor attack
                backdoored_inputs, backdoored_targets = attacks_flame.add_backdoor(inputs, targets, dataset)
...
def main(args):
    ...
    num_inputs, num_outputs, num_labels = data_loaders_flame.get_shapes(args.dataset)
    ...
    paraString = ('dataset: p' + str(args.p) + '_' + str(args.dataset) + ", server_pc: " + str(
        args.server_pc) + ", bias: " + str(args.bias)
                  + ", nworkers: " + str(args.nworkers) + ", net: " + str(args.net) + ", niter: " + str(
                args.niter) + ", lr: " + str(args.lr)
                  + ", batch_size: " + str(args.batch_size) + ", nbyz: " + str(args.nbyz) + ", attack: " + str(
                args.byz_type)
                  + ", aggregation: " + str(args.aggregation)
                  + ", Flame_epsilon: " + str(args.flame_epsilon) + ", Flame_delta: " + str(
                args.flame_delta) + ", Number_runs: " + str(args.nruns)
                  + ", MP-SPDZ: " + str(args.mpspdz) + ", Port: " + str(args.port) + ", Chunk_size: " + str(
                args.chunk_size)
                  + ", Protocol: " + args.protocol + ", Threads: " + str(args.threads) + ", Parallels: " + str(
                args.parallels)
    ...
    for run in range(1, args.nruns + 1):
        ...
        server_data, server_label, each_worker_data, each_worker_label = data_loaders_flame.assign_data(train_data,
                                                                                                        args.bias,
                                                                                                        device,
        ...
        # print(each_worker_label)
        #
        # print("Each Worker Data:")
        # for i, worker_data in enumerate(each_worker_data):
        #     print(f"Worker {i + 1} Data:")
        #     print(worker_data)

        print("Each Worker Label:")
        for i in enumerate(each_worker_label):
        #     print(f"Worker {i + 1} Label:")
        #     print(worker_label)
        ...
        if args.byz_type == "label_flipping_attack":
            each_worker_label = attacks_flame.label_flipping_attack(each_worker_label, args.nbyz, num_labels)
        elif args.byz_type == "scaling_attack":
            each_worker_data, each_worker_label = attacks_flame.scaling_attack_insert_backdoor(each_worker_data,
                                                                                         each_worker_label,
                                                                                         args.dataset, args.nbyz,
                                                                                         device)
        ...
        if args.mpspdz:
            ...
            server_process = subprocess.Popen(
                ["./run_aggregation.sh", args.script, args.full_filename, str(args.players)])
        ...
        with torch.no_grad():
            ...
            for e in range(args.niter):
                ...
                for i in range(args.nworkers):
                    minibatch = np.random.choice(list(range(each_worker_data[i].shape[0])), size=args.batch_size,
                                                 replace=False)
                ...
                if args.mpspdz:
                    aggregation_rules.mpspdz_aggregation(grad_list, net, args.lr, args.nbyz, byz, device,
                                                         param_num=num_params, port=args.port,
                                                         chunk_size=args.chunk_size, parties=args.players)
                    ...
                    aggregation_flame.flame(grad_list, net, args.lr, args.nbyz, byz, device, epsilon=args.flame_epsilon,
                                            delta=args.flame_delta)
                ...
                if (e + 1) % args.test_every == 0:
                    test_accuracy, test_success_rate = evaluate_accuracy(test_data, net, device,
                                                                         args.byz_type == "scaling_attack",
                                                                         args.dataset)
                    ...
                    if args.byz_type == "scaling_attack":
                        ...
                        wandb.log({'Iteration': e, 'Test Accuracy': test_accuracy, 'BA Success': test_success_rate})
        ...
        if args.byz_type == "scaling_attack":
            ...
            run, args.nruns, test_acc_list[-1], backdoor_success_list[-1]))
        else:
